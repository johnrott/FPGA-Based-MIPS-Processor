...
module DataMemory(Address, WriteData, Clk, MemWrite, MemRead, ReadData, memSize); 
    ...
    input [1:0] memSize;    // 00 IS WORD, 01 IS HALF, 11 IS BYTE
   ...
   always @(posedge Clk) begin
       if (MemWrite) begin
        case (memSize)
            ...
            2'b01: begin
                if(Address[1] == 1'b0)
                    Memory[word_address][15:0]  <= WriteData[15:0];
                else
                    Memory[word_address][31:16] <= WriteData[15:0];
                end
            2'b10: begin
                case(byte_off)
                    2'b00: begin
                        Memory[word_address][7:0] <= WriteData[7:0];
                    end
                    2'b01: begin
                        Memory[word_address][15:8] <= WriteData[7:0];
                    end
                    2'b10: begin
                        Memory[word_address][23:16] <= WriteData[7:0];
                    end
                    2'b11: begin
                        Memory[word_address][31:24] <= WriteData[7:0];
                    end
                endcase
            end
...
always @(negedge Clk) begin
  if (MemRead) begin
    case (memSize)
      2'b00: ReadData = Memory[word_address]; // LW
      2'b01: begin // LH (sign-extend)
        case (byte_off)
          2'b00: ReadData = {{16{Memory[word_address][15]}},  Memory[word_address][15:0]};
          2'b10: ReadData = {{16{Memory[word_address][31]}},  Memory[word_address][31:16]};
          default: ReadData = 32'b0; // misaligned half
        endcase
      end
      2'b10: begin // LB (sign-extend)
        case (byte_off)
